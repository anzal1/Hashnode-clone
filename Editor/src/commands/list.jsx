...
export const makeList = (state, api, insertBefore, dispatch) => {
  // Adjust the selection to encompass the whole word if the caret is inside one
  const allListElement = state.selectedText.split("\n");

  const result = allListElement.map((element) => {
    const Regexs = {
      "- ": /^[- ].*/,
      "2. ": /^[0-9. ].*/,
      "- [ ] ": /^- \[\ \]\ .*/,
    ...
    const regex1 = Regexs[insertBefore(null, 1)];
    if (element.match(regex1)) {
      const starting = insertBefore(null, 1).length;
      return element.slice(starting, element.length);
    }
  });

  if (!result.includes(undefined)) {
    api.replaceSelection(result.filter(Boolean).join("\n"));
...
export const unorderedListCommand = {
  ...
  value: "- ",
  buttonProps: {
    "aria-label": "Add un-ordered list (ctrl + shift + u)",
    title: "Add un-ordered list (ctrl + shift + u)",
  },
  icon: (
    <svg
